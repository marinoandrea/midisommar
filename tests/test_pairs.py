...
def _every_bar(
    pitch: int, n_bars: int, bpb: float = 4.0, channel: int = 0
) -> list[Note]:
...
def test_window_count_and_require_both():
    ...
    chunks = window_pairs(
        bass, drums, bpb=4.0, chunk_bars=1
    )  # 1-bar windows, hop = 1 bar
    ...
    only_both = window_pairs(
        bass, sparse_drums, bpb=4.0, chunk_bars=1, require_both=True
    )
...
def test_augment_leaves_drum_target_untouched():
    ...
    assert {c[0].pitch for c, _ in out} == {
        38,
        39,
        40,
        41,
...
def test_build_aligned_pairs_from_disk(tmp_path):
    ...
    pairs = build_aligned_pairs(
        tmp_path, "Bass", "Drums", chunk_bars=1, normalize_drums=False
    )
