...
from midisommar.data.midi import load_midi_file, save_notes_midi, transpose
...
def build_aligned_pairs(
    ...
    for song in sorted(set(ctx_songs) & set(tgt_songs)):
        ...
        if tgt_inst.lower() == "drums":
            if normalize_drums:
                from midisommar.data.drums import normalize_drums as _normalize

                tgt_notes = _normalize(tgt_notes)
            if regularize:
                from midisommar.data.drums import regularize_drums as _regularize
