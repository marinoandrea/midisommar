...
(``midisommar in``), so you can hear/verify the duet respond. Two terminals::
...
Then route ``midisommar out`` to a synth in Ableton to hear the model's response.
"""
...
from midisommar.data.midi import load_midi_dir, load_midi_file  # noqa: E402
from midisommar.realtime.clock import make_clock  # noqa: E402

DEFAULT_TARGET = "midisommar in"
...
def play_loop(
    ...
    *,
    ...
    ``clock`` (see :mod:`midisommar.realtime.clock`) supplies the beat position. Each loop pass is anchored to a
